// SPDX-FileCopyrightText: 2022 BlockCarbon Foundation MTÜ
// SPDX-License-Identifier: BUSL-1.1
// based on open source docs by Toucan:
...
import './CarbonOffsetsFactory.sol';
    ...
    CarbonOffsetsStorage
{
    ...
    modifier whenNotPaused() {
        address CarbonOffsetsFactoryAddress = IContractRegistry(
            contractRegistry
        ). CarbonOffsetsFactoryAddress();
        bool _paused =  CarbonOffsetsFactory(
            CarbonOffsetsFactoryAddress
        ).paused();
        require(!_paused, 'Error: carbon credit contract is paused');
        _;
    }
    ...
    function symbol() public view virtual override returns (string memory) {
        ...
        return
            string(
                abi.encodePacked('CO2-', globalProjectId, '-', vintageName)
            );
    }
        ...
        returns (ProjectData memory, VintageData memory)
    {
        ...
        address vc = I ContractRegistry(contractRegistry)
    ...
    function onERC721Received(
        ...
        require(
         ...
         CarbonOffsetsFactory  CO2Factory = CarbonOffsetsFactory(
        ...
        if (bridgeFeeReceiver == address(0x0)) {
            ...
            (uint256 feeAmount, uint256 feeBurnAmount) =  CO2Factory
                .getBridgeFeeAndBurnAmount(quantity);
            _mint(from, quantity - feeAmount);
            address bridgeFeeBurnAddress =  CO2Factory.bridgeFeeBurnAddress();
        ...
        returns (bool)
    {
        if (
            collection ==
            I ContractRegistry(contractRegistry)
    ...
    /// @dev Returns the remaining space in  CO2 contract before hitting the cap
    function getRemaining() public view returns (uint256 remaining) {
        uint256 cap = getDepositCap();
        remaining = cap - totalSupply();
    }

    /// @dev Returns the cap for  CO2s based on `totalVintageQuantity`
    ...
    function getDepositCap() public view returns (uint256) {
        ...
        /// @dev if totalVintageQuantity is not set (=0), re universal cap
        if (cap == 0) {
            return (2**256 - 1);
        } else {
            return cap;
    ...
    /// @notice Retirement/Cancellation of  CO2 tokens (the actual offsetting),
    ...
    /// @dev Internal function for the burning of  CO2 tokens
    function _retire(address account, uint256 amount) internal virtual {
        _burn(account, amount);
        retiredAmount[account] += amount;

        address CO2FactoryAddress = I ContractRegistry(contractRegistry)
            . CarbonOffsetsFactoryAddress();
         CarbonOffsetsFactory( CO2FactoryAddress).increaseTotalRetired(
            amount
        );
    ...
    function mintCertificate(
        ...
        address certAddr = I ContractRegistry(contractRegistry)
